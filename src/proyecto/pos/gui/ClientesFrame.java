...
public class ClientesFrame extends JFrame {
    ...
    private static final Color MORADO_BTN = new Color(107, 33, 168);
    ...
    private void cargarClientes() {
        ...
        ArrayList<Cliente> lista = (ArrayList<Cliente>) cliente_controller.listarClientes();
        for (Cliente c : lista) {
    ...
    private JPanel buildTableCard() {
        ...
        JLabel chd = new JLabel("Haz clic en Agregar para añadir  ·  Clic sobre el Tipo para modificarlo  ·  Selecciona y usa 'Ver Perfil CRM' — HU-08");
        ...
        modelo = new DefaultTableModel(
            new String[]{"ID","Tipo","Nombre","Apellido","DNI","Teléfono","Email","Puntos","Dirección","Fecha"}, 0
        ) {
        ...
        int[] widths = {0, 90, 160, 150, 110, 120, 240, 80, 220, 120};
    ...
    // ══════════════════════════════════════════════════════════════════════════
    //  BOTONES — con botón "Ver Perfil CRM" agregado (HU-08)
    // ══════════════════════════════════════════════════════════════════════════
    private JPanel buildBotones() {
        ...
        // HU-08 — botón Ver Perfil CRM
        JButton bCRM = btn("★  Ver Perfil CRM", MORADO_BTN);

        bE  .addActionListener(e -> eliminarFila());
        bEd .addActionListener(e -> { activarEdicion(); actualizarFila(filaEditando); });
        bG  .addActionListener(e -> guardar());
        bA  .addActionListener(e -> agregarFila());

        // HU-08: abrir PerfilClienteCRMFrame con el cliente ya seleccionado
        bCRM.addActionListener(e -> abrirPerfilCRM());

        p.add(bE); p.add(bEd); p.add(bG); p.add(bA); p.add(bCRM);
        return p;
    }

    // ── HU-08: Abrir PerfilClienteCRMFrame con cliente preseleccionado ────────
    private void abrirPerfilCRM() {
        int fila = tabla.getSelectedRow();
        if (fila < 0) {
            JOptionPane.showMessageDialog(this,
                "Selecciona un cliente para ver su Perfil CRM.",
        ...
        if (idObj == null || idObj.toString().trim().isEmpty()) {
            JOptionPane.showMessageDialog(this,
                "Este cliente aún no ha sido guardado en la BD.\nGuárdalo primero antes de ver su Perfil CRM.",
                "Cliente sin ID", JOptionPane.WARNING_MESSAGE);
            return;
        }

        int clienteId = Integer.parseInt(idObj.toString());
        SwingUtilities.invokeLater(() -> {
            PerfilClienteCRMFrame frame = new PerfilClienteCRMFrame(clienteId);
            frame.setVisible(true);
            dispose(); // cerrar ClientesFrame
        });
    }
    ...
    private void abrirModalTipo(int fila) {
        ...
        JPanel optNat = tipoOption("NATURAL", "Persona Natural", "DNI (8 dígitos)  ·  Nombre + Apellido requeridos", "NATURAL".equals(actual), bg);
        JPanel optEmp = tipoOption("EMPRESA", "Empresa", "RUC (11 dígitos)  ·  Razón Social  ·  Sin Apellido", "EMPRESA".equals(actual), bg);
        body.add(optNat);
    ...
    private JPanel tipoOption(String cmd, String titulo, String desc, boolean selected, ButtonGroup group) {
        ...
        txt.add(lTit); txt.add(Box.createVerticalStrut(3)); txt.add(lDesc);
        ...
        p.addMouseListener(ma); txt.addMouseListener(ma);
        lTit.addMouseListener(ma); lDesc.addMouseListener(ma);
        return p;
    }
    ...
    private void agregarFila() {
        ...
        modelo.addRow(new Object[]{null, "NATURAL", "", "", "", "", "", "0", "", hoy});
    ...
    private void eliminarFila() {
        ...
        if (idObj == null) {
            ...
            int op = JOptionPane.showConfirmDialog(this, "¿Deseas eliminar este cliente?", "Confirmar eliminación", JOptionPane.YES_NO_OPTION);
    ...
    private void guardar() {
        ...
        for (int i = 0; i < modelo.getRowCount(); i++) {
            ...
            if (idObj != null && !idObj.toString().trim().isEmpty() && !idObj.toString().trim().equals("0")) continue;
            ...
            Date fecha;
            try { fecha = fmt.parse(fechaStr); } catch (Exception ex) { err("Fecha inválida", i); return; }
            Cliente existente = cliente_controller.obtenerPorDni(dni);
            if (existente != null) { err("Cliente ya registrado", i); return; }
            try {
                cliente_controller.registrarCliente(esEmpresa ? "EMPRESA" : "NATURAL", nombre, apellido, dni, tel, email, direccion, fecha);
            } catch (Exception e) { err("Error al guardar: " + e.getMessage(), i); return; }
        }
        cargarClientes();
    ...
    private void actualizarFila(int fila) {
        ...
        if (idObj != null) {
            ...
            boolean esEmpresa = "EMPRESA".equals(tipo);
            int idx = Integer.parseInt(idObj.toString());
            ...
            Date fecha;
            try { fecha = fmt.parse(fechaStr); } catch (Exception ex) { err("Fecha inválida", fila); return; }
            try {
                cliente_controller.actualizarCliente(idx, esEmpresa ? "EMPRESA" : "NATURAL", nombre, apellido, dni, tel, email, direccion, fecha, 0);
            } catch (Exception e) { err("Error al guardar: " + e.getMessage(), fila); return; }
        }
        cargarClientes();
        JOptionPane.showMessageDialog(this, "Clientes guardados correctamente");
    }
    ...
    private void err(String msg, int row) {
        JOptionPane.showMessageDialog(this, msg + " — fila " + (row + 1), "Validación", JOptionPane.WARNING_MESSAGE);
    }
